...
MapTileSourceOSM::MapTileSourceOSM(OSMTileType tileType)
    : MapTileSource()
    , _tileType(tileType)
{
...
MapTileSourceOSM::~MapTileSourceOSM()
{
    qDebug() << this << this->name() << "Destructing";
}

QPointF MapTileSourceOSM::ll2qgs(const QPointF &ll, quint8 zoomLevel) const
{
...
QPointF MapTileSourceOSM::qgs2ll(const QPointF &qgs, quint8 zoomLevel) const
{
    const qreal tilesOnOneEdge = pow(2.0,zoomLevel);
    const quint16 tileSize = this->tileSize();
...
quint64 MapTileSourceOSM::tilesOnZoomLevel(quint8 zoomLevel) const
{
    return pow(4.0,zoomLevel);
}

quint16 MapTileSourceOSM::tileSize() const
{
    return 256;
}

quint8 MapTileSourceOSM::minZoomLevel(QPointF ll)
{
    Q_UNUSED(ll)
    return 0;
}

quint8 MapTileSourceOSM::maxZoomLevel(QPointF ll)
{
    Q_UNUSED(ll)
    return 18;
}

QString MapTileSourceOSM::name() const
{
    switch(_tileType)
    {
    case OSMTiles:
...
QString MapTileSourceOSM::tileFileExtension() const
{
...
void MapTileSourceOSM::fetchTile(quint32 x, quint32 y, quint8 z)
{
...
void MapTileSourceOSM::handleNetworkRequestFinished()
{
