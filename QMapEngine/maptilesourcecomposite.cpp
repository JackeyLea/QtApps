...
MapTileSourceComposite::MapTileSourceComposite()
    : MapTileSource()
{
...
QPointF MapTileSourceComposite::ll2qgs(const QPointF &ll, quint8 zoomLevel) const
{
...
QPointF MapTileSourceComposite::qgs2ll(const QPointF &qgs, quint8 zoomLevel) const
{
    QMutexLocker lock(_globalMutex);
    if (_childSources.isEmpty())
    {
...
quint64 MapTileSourceComposite::tilesOnZoomLevel(quint8 zoomLevel) const
{
    QMutexLocker lock(_globalMutex);
    if (_childSources.isEmpty())
        return 1;
...
quint16 MapTileSourceComposite::tileSize() const
{
    QMutexLocker lock(_globalMutex);
    if (_childSources.isEmpty())
        return 256;
...
quint8 MapTileSourceComposite::minZoomLevel(QPointF ll)
{
    QMutexLocker lock(_globalMutex);
...
quint8 MapTileSourceComposite::maxZoomLevel(QPointF ll)
{
    QMutexLocker lock(_globalMutex);
...
QString MapTileSourceComposite::name() const
{
    return "Composite Tile Source";
}

QString MapTileSourceComposite::tileFileExtension() const
{
    return ".jpg";
}

void MapTileSourceComposite::addSourceTop(QSharedPointer<MapTileSource> source, qreal opacity)
{
    QMutexLocker lock(_globalMutex);
    if (source.isNull())
        return;
...
void MapTileSourceComposite::addSourceBottom(QSharedPointer<MapTileSource> source, qreal opacity)
{
    QMutexLocker lock(_globalMutex);
    if (source.isNull())
        return;
...
void MapTileSourceComposite::moveSource(int from, int to)
{
    if (from < 0 || to < 0)
        return;
...
void MapTileSourceComposite::removeSource(int index)
{
    QMutexLocker lock(_globalMutex);
...
int MapTileSourceComposite::numSources() const
{
    QMutexLocker lock(_globalMutex);
    return _childSources.size();
}

QSharedPointer<MapTileSource> MapTileSourceComposite::getSource(int index) const
{
    QMutexLocker lock(_globalMutex);
    if (index < 0 || index >= _childSources.size())
...
qreal MapTileSourceComposite::getOpacity(int index) const
{
    QMutexLocker lock(_globalMutex);
    if (index < 0 || index >= _childSources.size())
        return 0.0;
...
void MapTileSourceComposite::setOpacity(int index, qreal opacity)
{
...
bool MapTileSourceComposite::getEnabledFlag(int index) const
{
    QMutexLocker lock(_globalMutex);
    if (index < 0 || index >= _childSources.size())
        return 0.0;
...
void MapTileSourceComposite::setEnabledFlag(int index, bool isEnabled)
{
    QMutexLocker lock(_globalMutex);
    if (index < 0 || index >= _childSources.size())
        return;
...
void MapTileSourceComposite::fetchTile(quint32 x, quint32 y, quint8 z)
{
    QMutexLocker lock(_globalMutex);
...
void MapTileSourceComposite::handleTileRetrieved(quint32 x, quint32 y, quint8 z)
{
    QMutexLocker lock(_globalMutex);
...
void MapTileSourceComposite::clearPendingTiles()
{
...
void MapTileSourceComposite::doChildThreading(QSharedPointer<MapTileSource> source)
{
    if (source.isNull())
        return;
