...
MapView::~MapView()
{
    ...
    foreach(MapTileObject * tileObject, _tileObjects)
    {
...
void MapView::centerOn(qreal longitude, qreal latitude)
{
    this->centerOn(QPointF(longitude,latitude));
}

void MapView::centerOn(const MapObject *item)
{
    if (item)
        this->centerOn(item->pos());
}

QPointF MapView::mapToScene(const QPoint viewPos) const
{
    if (_tileSource.isNull())
    {
...
void MapView::setDragMode(DragMode mode)
{
    _dragMode = mode;

    QGraphicsView::DragMode qgvDragMode;
    if (_dragMode == MapView::NoDrag)
        qgvDragMode = QGraphicsView::NoDrag;
    else if (_dragMode == MapView::ScrollHandDrag)
...
void MapView::setScene(MapScene *scene)
{
    ...
    */
    ...
    MapScenePrivate * childScene = new MapScenePrivate(scene,
                                                                   this,
                                                                   this);
    ...
    MapViewPrivate * childView = new MapViewPrivate(childScene, this);
    connect(childView,
...
void MapView::setTileSource(QSharedPointer<MapTileSource> tSource)
{
    _tileSource = tSource;
...
void MapView::setZoomLevel(quint8 nZoom, ZoomMode zMode)
{
    if (_tileSource.isNull())
        return;
...
void MapView::zoomIn(ZoomMode zMode)
{
    if (_tileSource.isNull())
        return;
...
void MapView::zoomOut(ZoomMode zMode)
{
    if (_tileSource.isNull())
        return;
...
void MapView::rotate(qreal rotation)
{
    _childView->rotate(rotation);
}

void MapView::handleChildMouseDoubleClick(QMouseEvent *event)
{
    event->setAccepted(false);
}

void MapView::handleChildMouseMove(QMouseEvent *event)
{
    event->setAccepted(false);
}

void MapView::handleChildMousePress(QMouseEvent *event)
{
    event->setAccepted(false);
}

void MapView::handleChildMouseRelease(QMouseEvent *event)
{
    event->setAccepted(false);
}

void MapView::handleChildViewContextMenu(QContextMenuEvent *event)
{
    event->setAccepted(false);
}
...
void MapView::doTileLayout()
{
    ...
    QQueue<MapTileObject *> freeTiles;
    ...
    for (qint32 x = xc; x < xMax; x++)
    {
        for (qint32 y = yc; y < yMax; y++)
        {
            ...
            {
                MapTileObject * tileObject = new MapTileObject(tileSize);
            ...
            MapTileObject * tileObject = freeTiles.dequeue();
...
void MapView::resetQGSSceneSize()
{
    if (_tileSource.isNull())
        return;
