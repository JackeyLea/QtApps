...
MapTileSourceGrid::MapTileSourceGrid()
    : MapTileSource()
{
...
QPointF MapTileSourceGrid::ll2qgs(const QPointF &ll, quint8 zoomLevel) const
{
...
QPointF MapTileSourceGrid::qgs2ll(const QPointF &qgs, quint8 zoomLevel) const
{
    const qreal tilesOnOneEdge = pow(2.0,zoomLevel);
    const quint16 tileSize = this->tileSize();
...
quint64 MapTileSourceGrid::tilesOnZoomLevel(quint8 zoomLevel) const
{
    return pow(4.0,zoomLevel);
}

quint16 MapTileSourceGrid::tileSize() const
{
    return 256;
}

quint8 MapTileSourceGrid::minZoomLevel(QPointF ll)
{
    Q_UNUSED(ll)
    return 0;
}

quint8 MapTileSourceGrid::maxZoomLevel(QPointF ll)
{
    Q_UNUSED(ll)
    return 50;
}

QString MapTileSourceGrid::name() const
{
    return "Grid Tiles";
}

QString MapTileSourceGrid::tileFileExtension() const
{
    return ".png";
}

void MapTileSourceGrid::fetchTile(quint32 x, quint32 y, quint8 z)
{
